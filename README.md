...
- **标的库**：分页浏览/搜索全市场股票与场内基金（股票/基金双 Tab，按代码或名称模糊查找）。
- **个股详情**：分钟级轮询近实时报价 + 分时图 + 技术/基本面/资金面"注意点"标签。
- **AI 智能分析**：个股详情页一键调用大模型（默认 DeepSeek，OpenAI 兼容可换豆包/通义等），
  把系统已算好的报价/技术/基本面/资金面数据生成结构化「决策报告」（结论/评分/趋势/风险/检查清单）。
  数据先标注可用性再喂模型、强制 JSON 输出，规避"数据缺失却硬下结论"。需配 API Key（见下文），未配置则按钮自动隐藏。
- **模拟交易**：虚拟资金（默认 100 万）按最新价练习买卖，整手 + 真实 A 股费用（ETF 免印花税），
  持仓浮动盈亏、已实现盈亏、净值曲线，按用户隔离；纯模拟、不接券商。
- **历史记录**：回测/寻优/选股运行自动落地，可回看入参与结果做对比。
- **登录与收藏**：用户名+密码注册登录（轻量本地 SQLite），每个用户维护各自的自选股
  收藏与模拟交易账户；个股详情页一键 ☆ 收藏，「我的收藏」页集中查看。首次使用在登录页注册即可。
...
只有想用「AI 智能分析」时才需要，配好后个股详情页才会出现「生成 AI 分析报告」按钮：

```bash
cp .env.example .env     # 复制模板
# 编辑 .env，填入 DEEPSEEK_API_KEY=sk-xxxx（DeepSeek 平台申请）
```

`.env` 已被 gitignore，不会入库。换其他 OpenAI 兼容服务（豆包/通义/OpenAI 等）只需在 `.env` 改
`LLM_BASE_URL` / `LLM_MODEL` / `LLM_API_KEY`，无需改代码，详见 `.env.example`。
...
之后复用构建产物秒起。启动后浏览器打开 **http://127.0.0.1:8000**（首次需在登录页注册账号），
顶部导航在各页面间切换：**策略回测 / 标的库 / 模拟交易 / 截面选股 / 个股详情 / 我的收藏 / 历史记录**；
API 文档在 `/docs`。
