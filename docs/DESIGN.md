...
在现有"数据 → 因子/策略 → 回测 → 个股详情"能力之上，叠加一层**基于大模型的智能分析**。
定位是"把系统已经算好的结构化数据翻译成人话决策"，**不是**让模型自由发挥写行情小作文。

设计目标与边界：
- **复用既有数据层**：报价（`quotes`）、日线（`fetcher`）、基本面（`fundamentals`）、资金面（`altdata`）、
  注意点信号（`signals`）已经齐备，AI 层只做"取数 → 校验 → 拼提示词 → 调用 → 渲染"，不重复造数据源。
- **供应商无关**：走 OpenAI 兼容协议（`/chat/completions`），DeepSeek / 豆包 / 通义 / OpenAI 等
  一套代码全覆盖；默认 DeepSeek。Key 与 `base_url`/`model` 经环境变量 / 根目录 `.env` 注入，不入库。
- **数据先校验再喂模型**：每个数据源的可用性标注为 `available/partial/missing`，缺失字段明确告知模型
  "数据不足"，杜绝"业绩缺失却算出 PE 偏低""缩量与放大并存"这类自相矛盾。
- **结构化输出**：要求模型返回 JSON（结论/评分/趋势/置信度/核心要点/风险/催化/操作建议/检查清单/
  数据局限），前端按字段渲染，可控、可审计；而非一段难以校验的长文。
- **纯研究辅助**：报告页醒目标注"不构成投资建议"，与系统整体定位一致。

模块与接口：
- `quant/ai/config.py`：读取根目录 `.env`（零依赖极简解析）+ 环境变量，给出 `base_url/api_key/model`
  与 `is_configured()`；默认 DeepSeek（`https://api.deepseek.com` / `deepseek-chat`）。
- `quant/ai/client.py`：基于 `httpx` 的 OpenAI 兼容 chat 客户端，支持 JSON 输出模式，超时/错误安全。
- `quant/ai/report.py`：`build_data_pack(symbol)` 汇总并校验数据；`generate_stock_report(symbol)`
  拼提示词、调用模型、解析校验 JSON，附 `meta`（模型、生成时间、数据可用性）。
- 后端 `backend/routers/ai.py`：`GET /ai/status`（是否已配置，供前端决定显隐按钮）、
  `POST /ai/report`（同步生成个股报告，单股耗时可接受，前端展示加载态）。
- 前端「个股详情」页新增「AI 智能分析」面板：一键生成 + 结构化卡片渲染（结论徽章/评分/风险/检查清单）。

后续可在此地基上继续叠加（本次不做）：AI 解读回测结果、AI 问股 Agent（工具调用）、AI 大盘复盘。
...
**Phase 10 — AI 智能分析（地基 + 个股报告）**
- `quant/ai/`：OpenAI 兼容 LLM 客户端 + 配置（默认 DeepSeek，`.env` 注入）+ 数据包构建 + 个股报告生成。
- 后端 `/ai/status`、`/ai/report`；前端「个股详情」页 AI 面板。
- 共享地基为后续 AI 解读回测、问股 Agent、大盘复盘复用。
...
- **Phase 10 — AI 智能分析（地基 + 个股报告）**（2026-06-29）：
  - `quant/ai/`：`config.py`（根目录 `.env` 极简加载 + 环境变量，默认 DeepSeek，`is_configured()`）、
    `client.py`（`httpx` OpenAI 兼容 `/chat/completions`，JSON 输出模式，超时/错误安全降级）、
    `report.py`（`build_data_pack` 汇总报价/日线技术指标/基本面/资金面/注意点并标注
    `available/partial/missing`；`generate_stock_report` 拼提示词、调用模型、解析校验 JSON）。
  - 关键设计：**数据先校验再喂模型 + 强制结构化 JSON 输出**，规避点评友商报告时发现的
    "业绩缺失却算 PE 偏低""缩量与放大 71 倍并存""盘后误判"等自相矛盾问题。
  - 后端 `backend/routers/ai.py`：`GET /ai/status`（是否配置）、`POST /ai/report`（同步生成）。
  - 前端「个股详情」页新增「AI 智能分析」面板：未配置时提示填 `.env`；已配置时一键生成，
    结构化渲染结论徽章/评分/趋势/核心要点/风险/催化/操作建议/检查清单/数据局限。
  - 测试：`tests/` 增至 **89 用例全绿**（新增 AI 数据包可用性标注、报告解析、缺失降级，全部离线
    monkeypatch LLM 客户端与数据源，不发真实网络请求）。
  - 共享地基为后续 AI 解读回测结果、AI 问股 Agent、AI 大盘复盘复用。
- 仍待后续（设计已记录，本次未做）：scheduler 定时增量更新、多标的组合、美股数据源、龙虎榜/十大流通股东等更多另类数据接口；AI 解读回测结果 / 问股 Agent / 大盘复盘。
